...
import csv
# filepath ="/Users/ashara/Documents/Study/Research/Dissertation/One Drive/OneDrive - University of Texas at Arlington/Dissertation/data_files/results/FP_Growth_test_Drug_1.csv"
filepath ="/Users/ashara/Documents/Study/Research/Dissertation/One Drive/OneDrive - University of Texas at Arlington/Dissertation/data_files/results/FP_Growth_Synpuf.csv"
...
with open(filepath) as csvfile:
    ...
    for row_2_attr in csv_reader:
        ...
        #Step 1
        if "Symptom" in l_2 and "Diagnosis" not in l_2 and "Procedure" not in l_2 and "Drug" not in l_2\
                and "Procedure" in r_2 and "Symptom" not in r_2 and "Diagnosis" not in r_2 and "Drug" not in r_2:
            temp_list.append(l_2)
            temp_list.append(r_2)
            s = sorted(temp_list)
            left = s[0] + ", " + s[1]
            ...
            for row_3_attr in csv_reader:
                r_3 = row_3_attr[1]
                #Step 2
                if "Diagnosis" in r_3 and "Procedure" not in r_3 and "Symptom" not in r_3:
                    conf_3 = float(row_3_attr[2])
                    lift_3 = float(row_3_attr[3])
                    attr = row_3_attr[0].split(",")
                    attr = [i.strip() for i in attr]
                    if len(attr) == 2:
                        ...
                        if left == l_3:
                            ...
                            # s = sorted(attr)
                            # left = s[0] + ", " + s[1] + ", " + s[2]
                            if (lift_3*conf_3)>=(lift_2*conf_2):
                                print("Left: ", l_3)
                                print("Right: ", r_3)
                                print("Conf 1: ", conf_2)
                                print("Conf 2: ", conf_3)
